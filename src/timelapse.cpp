...
std::string formatTimelapseName(const std::string &pattern, int i) {
    ...
    if (hasIntConv) {
        ...
        if (specStart == std::string::npos) {
            throw std::runtime_error("internal error: could not locate int conversion");
        }
        ...
        while (fp < fmtSpec.size() && fmtSpec[fp] >= '0' && fmtSpec[fp] <= '9') {
            ...
            if (width > 511) {
                throw std::invalid_argument(
                    "output pattern width too large (max 511)");
            }
            ++fp;
        }
        ...
        if (result.size() >= 512) {
            throw std::runtime_error("output pattern produces a filename too long (>= 512 chars)");
        }
        if (!isSafeFilePath(result)) {
            throw std::runtime_error("formatted timelapse filename contains unsafe path components");
        }
        return result;
    }
...
#ifdef _WIN32
    if (std::localtime_s(&tm, &t) == 0) tmPtr = &tm;
#else
    ...
    if (!isSafeFilePath(result)) {
        throw std::runtime_error("formatted timelapse filename contains unsafe path components (.. or control characters)");
    }
    return result;
}
