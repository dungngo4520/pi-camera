...
bool St7735Display::init(const DisplayConfig &cfg) {
    ...
    bool ok = true;
    if (ok && !sendCommand(SWRESET)) ok = false;
    if (ok) std::this_thread::sleep_for(std::chrono::milliseconds(150)); // 150ms
    if (ok && !sendCommand(SLPOUT)) ok = false;
    if (ok) std::this_thread::sleep_for(std::chrono::milliseconds(120)); // 120ms

    // Pixel format: 16-bit RGB565
    uint8_t colmod = 0x05;
    if (ok && !sendCommand(COLMOD)) ok = false;
    if (ok && !sendData(&colmod, 1)) ok = false;
    ...
    uint8_t madctl = 0;
    if (ok && !sendCommand(MADCTL)) ok = false;
    if (ok) {
        ...
        if (!sendData(&madctl, 1)) ok = false;
    }

    // Set address window
    if (ok && !setAddrWindow()) ok = false;

    // Display on
    if (ok && !sendCommand(DISPON)) ok = false;
    if (ok) std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 100ms
...
bool St7735Display::sendCommand(uint8_t cmd) {
    if (spiFd_ < 0 || !gpioReq_) return false;
    auto *req = gpioReq_;
    // DC=0 for command
    ...
    if (!spiTransfer(spiFd_, tr)) return false;
    return true;
}
...
bool St7735Display::setAddrWindow(int x, int y, int w, int h) {
    ...
    if (!sendCommand(CASET)) return false;
    if (!sendData(colData, 4)) return false;
    ...
    if (!sendCommand(RASET)) return false;
    if (!sendData(rowData, 4)) return false;
    return true;
}
...
bool St7735Display::blit(const uint8_t *rgb565) {
    ...
    if (!setAddrWindow()) return false;
    if (!sendCommand(RAMWR)) return false;
        ...
        if (cfg_.rotation == 270) {
            for (uint32_t y = 0; y < cfg_.height; ++y) {
                for (uint32_t x = 0; x < cfg_.width; ++x) {
                    ...
                    size_t dstIdx = (static_cast<size_t>(cfg_.width - 1 - x) *
                                     cfg_.height + y) * 2;
    ...
    for (size_t offset = 0; offset < totalBytes; offset += chunkSize) {
        ...
        if (!sendData(sendBuf + offset, len)) return false;
    }
    return true;
}

bool St7735Display::blitRegion(const uint8_t *rgb565, uint32_t srcW, uint32_t srcH,
                                int srcX, int srcY, int dispX, int dispY,
                                int w, int h) {
    if (spiFd_ < 0) return false;
    ...
    if (static_cast<int64_t>(srcX) + w > static_cast<int64_t>(srcW)) return false;
    if (static_cast<int64_t>(srcY) + h > static_cast<int64_t>(srcH)) return false;
    ...
    if (dispX < 0 || dispY < 0) return false;
    if (needsTranspose_) {
        if (static_cast<int64_t>(dispX) + h > cfg_.width) return false;
        if (static_cast<int64_t>(dispY) + w > cfg_.height) return false;
    } else {
        if (static_cast<int64_t>(dispX) + w > cfg_.width) return false;
        if (static_cast<int64_t>(dispY) + h > cfg_.height) return false;
    }
    ...
    if (needsTranspose_) {
        int winW = h, winH = w;
        if (!setAddrWindow(dispX, dispY, winW, winH)) return false;
        if (!sendCommand(RAMWR)) return false;
        ...
        for (size_t offset = 0; offset < regionBytes; offset += chunkSize) {
            ...
            if (!sendData(transposeBuf_.data() + offset, len)) return false;
        }
        return true;
    }

    // Normal (no rotation): set address window for the sub-region.
    if (!setAddrWindow(dispX, dispY, w, h)) return false;
    if (!sendCommand(RAMWR)) return false;
    ...
    const size_t chunkSize = 4096;
    for (int row = 0; row < h; ++row) {
        const uint8_t *srcRow = rgb565 +
            (static_cast<size_t>(srcY + row) * srcW + srcX) * 2;
        size_t rowBytes = static_cast<size_t>(w) * 2;

        if (rowBytes <= chunkSize) {
            // If the row fits in one chunk, send directly
            if (!sendData(srcRow, rowBytes)) return false;
        } else {
            ...
            for (size_t offset = 0; offset < rowBytes; offset += chunkSize) {
                ...
                if (!sendData(srcRow + offset, len)) return false;
            }
        }
    }
    return true;
...
void St7735Display::flash() {
    ...
    try {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(80)); // 80ms white flash
