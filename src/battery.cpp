...
static const VoltagePoint kLipoCurve[] = {
    {4.20, 100}, {4.10, 90}, {4.00, 80}, {3.90, 70}, {3.80, 55}, {3.70, 40},
    {3.60, 25},  {3.50, 15}, {3.40, 8},  {3.30, 5},  {3.00, 0},
};
...
uint16_t buildConfig(uint8_t channel, uint16_t pgaGain) {
  ...
  cfg |=
      static_cast<uint16_t>((0x4 | (channel & 0x3)) << 12); // MUX: single-ended
...
bool BatteryMonitor::init(const BatteryConfig &cfg) {
  ...
  switch (cfg_.pgaGain) {
  case 0x0000:
    lsbMillivolts_ = 0.1875;
    break; // ±6.144V
  case 0x0001:
    lsbMillivolts_ = 0.125;
    break; // ±4.096V
  case 0x0002:
    lsbMillivolts_ = 0.0625;
    break; // ±2.048V
  case 0x0003:
    lsbMillivolts_ = 0.03125;
    break; // ±1.024V
  default:
    std::cerr << "Battery: invalid PGA gain 0x" << std::hex << cfg_.pgaGain
              << std::dec << " (only 0-3 supported)\n";
    return false;
  }
  ...
  if (cfg_.i2cAddress < 0x03 || cfg_.i2cAddress > 0x77) {
    std::cerr << "Battery: invalid I2C address 0x" << std::hex
              << static_cast<unsigned>(cfg_.i2cAddress) << std::dec
              << " (must be 0x03–0x77)\n";
    close(fd_);
    fd_ = -1;
    return false;
  ...
  std::this_thread::sleep_for(
      std::chrono::milliseconds(10)); // 10ms > 1/128s ≈ 7.8ms

  char addrBuf[8];
  std::snprintf(addrBuf, sizeof(addrBuf), "%02X",
                static_cast<unsigned>(cfg_.i2cAddress));
  std::cout << "Battery: ADS1115 at 0x" << addrBuf << " initialized (channel "
            << static_cast<int>(cfg_.channel) << ", LSB=" << lsbMillivolts_
            << "mV)\n";
  return true;
}
...
BatteryReading BatteryMonitor::read() {
  ...
  if (raw > 0x7FFF) {
    std::cerr << "Battery: out-of-range ADC reading (0x" << std::hex << raw
              << std::dec << ")\n";
    return result;
  }
