...
FtState &ftState() {
    ...
    std::call_once(flag, [&] {
        if (FT_Init_FreeType(&state.library)) return;
        ...
        if (FT_Set_Pixel_Sizes(state.face, 0, 8) != 0) return;
        ...
        if (FT_Load_Char(state.face, 'M', FT_LOAD_RENDER) == 0) {
            ...
            if (state.advanceX < 1) state.advanceX = 6;
        }
...
const GlyphCache *getGlyph(unsigned long ch) {
    ...
    if (gc.advanceX < 1) gc.advanceX = ft.advanceX;
...
static constexpr int kGlyphTableLen = sizeof(kGlyphTable) / sizeof(kGlyphTable[0]);
...
void drawCharScaled(uint8_t *rgb565, uint32_t fbW, uint32_t fbH,
                    int x, int y, char ch, int scale,
                    uint16_t fg, uint16_t bg, bool transparent) {
    if (scale <= 0) return;  // scale 0 = draw nothing
    if (scale == 1) {
        drawChar(rgb565, fbW, fbH, x, y, ch, fg, bg, transparent);
        return;
    }
