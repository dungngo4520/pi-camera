...
std::optional<uint8_t> parseAddr(std::string_view v) {
  ...
  if (errno != 0 || *end != '\0' || val < 0 || val > 0xFF)
    return std::nullopt;
...
void applyHardwareKey(HardwareConfig &cfg, std::string_view key,
                      std::string_view val, const std::string &path,
                      int lineNum) {
  auto boolKey = [&](const char *name, bool &field) {
    ...
    if (b)
      ...
      std::cerr << path << ":" << lineNum << ": invalid " << name
                << " (true/false)\n";
  };
  auto uintKey = [&](const char *name, uint32_t &field, uint32_t lo,
                     uint32_t hi) {
  ...
  if (key == "spi_device") {
    ...
    if (a)
      cfg.batteryI2cAddress = *a;
    else
      std::cerr << path << ":" << lineNum << ": invalid battery_addr\n";
    ...
    if (fps && *fps >= 1 && *fps <= 120)
      ...
      std::cerr << path << ":" << lineNum << ": invalid preview_fps (1-120)\n";
