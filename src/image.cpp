...
static void nv12ToRgbRowPair_neon(const uint8_t *yRow0, const uint8_t *yRow1,
  ...
  while (x + 16 <= w) {
    ...
    convert8pixels_neon(vld1_u8(&yRow0[x]), u_dup.val[0], v_dup.val[0],
                        &rgb0[x * 3]);
    if (haveRow1) {
      convert8pixels_neon(vld1_u8(&yRow1[x]), u_dup.val[0], v_dup.val[0],
                          &rgb1[x * 3]);
    }

    // Second 8 pixels (high half).
    convert8pixels_neon(vld1_u8(&yRow0[x + 8]), u_dup.val[1], v_dup.val[1],
                        &rgb0[(x + 8) * 3]);
    if (haveRow1) {
      convert8pixels_neon(vld1_u8(&yRow1[x + 8]), u_dup.val[1], v_dup.val[1],
                          &rgb1[(x + 8) * 3]);
    }
    x += 16;
  }
  ...
  if (x < w) {
    nv12ToRgbRowPair_scalar(yRow0 + x, yRow1 + x, uvRow + x, rgb0 + x * 3,
                            rgb1 + x * 3, w - x, stride - x, haveRow1);
  }
}
...
std::vector<uint8_t> nv12ToRgb(const uint8_t *y, const uint8_t *uv, uint32_t w,
  ...
  if (!checkedMul(static_cast<size_t>(stride), h, needY))
    return {};
  if (!checkedMul(static_cast<size_t>(stride), (h + 1) / 2, needUv))
    return {};
  if (ySize < needY || uvSize < needUv)
    return {};
  ...
  if (nThreads <= 1 || h < 8) {
    for (uint32_t yRow = 0; yRow < h; yRow += 2) {
      ...
      nv12ToRgbRowPair(y + yOff0, haveRow1 ? y + yOff1 : y + yOff0, uv + uvOff,
                       rgb.data() + rgbOff0,
  ...
  for (unsigned t = 0; t < nThreads; ++t) {
    ...
    threads.emplace_back([&, startPair, endPair] {
      for (uint32_t p = startPair; p < endPair; ++p) {
        ...
        size_t rgbOff0 = static_cast<size_t>(yRow) * w * 3;
        size_t rgbOff1 = static_cast<size_t>(yRow + 1) * w * 3;
        nv12ToRgbRowPair(y + yOff0, haveRow1 ? y + yOff1 : y + yOff0,
                         uv + uvOff, rgb.data() + rgbOff0,
                         haveRow1 ? rgb.data() + rgbOff1 : rgb.data() + rgbOff0,
                         w, stride, haveRow1);
      }
    });
...
bool nv12ToRgb565Scaled(const uint8_t *y, const uint8_t *uv, uint32_t srcW,
  ...
  size_t needY = 0;
  size_t needUv = 0;
  if (!checkedMul(static_cast<size_t>(stride), srcH, needY))
    return false;
  if (!checkedMul(static_cast<size_t>(stride), (srcH + 1) / 2, needUv))
    return false;
  if (ySize < needY || uvSize < needUv)
    return false;
  size_t requiredOut = 0;
  if (!checkedMul(static_cast<size_t>(dispW), dispH, requiredOut) ||
      !checkedMul(requiredOut, 2, requiredOut))
    return false;
  if (outSize < requiredOut)
    return false;
...
std::vector<uint8_t> downscaleNv12(const uint8_t *y, const uint8_t *uv,
                                   ...
                                   int factor) {
  if (!y || !uv || srcW == 0 || srcH == 0 || stride < srcW)
    return {};
  ...
  size_t outYSize = 0;
  size_t outUvSize = 0;
  if (!checkedMul(static_cast<size_t>(outW), outH, outYSize))
    return {};
  if (!checkedMul(static_cast<size_t>(outW), outH / 2, outUvSize))
    return {};
...
std::vector<uint8_t> cropNv12(const uint8_t *y, const uint8_t *uv,
  ...
  size_t outYSize = 0;
  size_t outUvSize = 0;
  if (!checkedMul(static_cast<size_t>(cropW), cropH, outYSize))
    return {};
  if (!checkedMul(static_cast<size_t>(cropW), cropH / 2, outUvSize))
    return {};
  std::vector<uint8_t> result(outYSize + outUvSize);
  uint8_t *outY = result.data();
  uint8_t *outUv = result.data() + outYSize;
  ...
  for (uint32_t r = 0; r < cropH / 2; ++r) {
    const uint8_t *srcRow =
        uv + static_cast<size_t>(cropY / 2 + r) * stride + cropX;
