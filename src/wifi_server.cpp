...
template <typename T> std::string jsonNum(std::string_view key, T val) {
  std::ostringstream oss;
  oss << '"' << key << "\":" << val;
  return oss.str();
}
...
template <typename T> std::optional<T> jsonNumValue(std::string_view s) {
  if (s.empty())
    return std::nullopt;
  try {
...
std::optional<DriveMode> parseDriveModeJson(std::string_view s) {
  if (s == "single")
    return DriveMode::Single;
  if (s == "selftimer")
    return DriveMode::SelfTimer;
  if (s == "bracket")
    return DriveMode::Bracket;
  if (s == "timelapse")
    return DriveMode::Timelapse;
  if (s == "continuous")
...
std::optional<BracketType> parseBracketTypeJson(std::string_view s) {
  if (s == "ae")
    return BracketType::AE;
  if (s == "wb")
    return BracketType::WB;
  if (s == "iso")
    return BracketType::ISO;
  return std::nullopt;
}
...
std::string statusToJson(int batteryPercent, const CameraSettings &s,
  ...
  int iso =
      (s.analogueGain > 0) ? static_cast<int>(s.analogueGain * 100.0f) : 0;
  oss << jsonNum("iso", iso) << ',';
  oss << jsonNum("shutterUs", s.shutterUs) << ',';
...
std::string fileListingHtml(const std::vector<std::string> &files) {
  ...
  for (const auto &f : files) {
    ...
    oss << "<li><a href=\"/file/" << base << "\">" << base << "</a></li>";
  }
  if (files.empty()) {
...
void applySettingsJson(const std::string &json, CameraSettings &s) {
  if (auto v = jsonFindValue(json, "driveMode"))
    if (auto d = parseDriveModeJson(*v))
      s.driveMode = *d;
  if (auto v = jsonFindValue(json, "exposureMode"))
    if (auto e = parseExposureModeJson(*v))
      s.exposureMode = *e;
  if (auto v = jsonBoolValue(jsonFindValue(json, "aeEnable").value_or("")))
    s.aeEnable = *v;
  if (auto v =
          jsonNumValue<uint64_t>(jsonFindValue(json, "shutterUs").value_or("")))
    s.shutterUs = *v;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "analogueGain").value_or("")))
    s.analogueGain = *v;
  if (auto v = jsonNumValue<float>(
          jsonFindValue(json, "exposureValue").value_or("")))
    s.exposureValue = *v;
  ...
  if (auto v = jsonNumValue<int>(jsonFindValue(json, "flickerHz").value_or("")))
    s.flickerHz = *v;
  if (auto v = jsonNumValue<uint32_t>(
          jsonFindValue(json, "timerDuration").value_or("")))
    s.timerDuration = *v;
  if (auto v = jsonFindValue(json, "bracketType"))
    if (auto b = parseBracketTypeJson(*v))
      s.bracketType = *b;
  if (auto v = jsonNumValue<int>(
          jsonFindValue(json, "timelapseInterval").value_or("")))
    s.timelapseInterval = *v;
  if (auto v =
          jsonNumValue<int>(jsonFindValue(json, "timelapseCount").value_or("")))
    s.timelapseCount = *v;
  if (auto v = jsonFindValue(json, "captureFormat"))
    if (auto f = parseOutputFormat(*v))
      s.captureFormat = *f;
  if (auto v =
          jsonNumValue<int>(jsonFindValue(json, "jpegQuality").value_or("")))
    s.jpegQuality = *v;
  if (auto v = jsonNumValue<int>(jsonFindValue(json, "pngLevel").value_or("")))
    s.pngLevel = *v;
  if (auto v = jsonFindValue(json, "imageSize"))
    if (auto is = parseImageSizeJson(*v))
      s.imageSize = *is;
  if (auto v = jsonFindValue(json, "aspectRatio"))
    if (auto a = parseAspectJson(*v))
      s.aspectRatio = *a;
  if (auto v = jsonBoolValue(jsonFindValue(json, "awbEnable").value_or("")))
    s.awbEnable = *v;
  if (auto v = jsonFindValue(json, "awbMode"))
    s.awbMode = *v;
  if (auto v = jsonNumValue<int>(jsonFindValue(json, "wbKelvin").value_or("")))
    s.wbKelvin = *v;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "wbRedGain").value_or("")))
    s.wbRedGain = *v;
  if (auto v =
  ...
  if (auto v = jsonFindValue(json, "pictureStyle"))
    if (auto p = parsePictureStyleJson(*v))
      s.pictureStyle = *p;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "brightness").value_or("")))
    s.brightness = *v;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "contrast").value_or("")))
    s.contrast = *v;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "saturation").value_or("")))
    s.saturation = *v;
  if (auto v =
          jsonNumValue<float>(jsonFindValue(json, "sharpness").value_or("")))
    s.sharpness = *v;
  if (auto v = jsonNumValue<int>(
          jsonFindValue(json, "displayBrightness").value_or("")))
    s.displayBrightness = *v;
  if (auto v =
          jsonNumValue<int>(jsonFindValue(json, "focusMagnify").value_or("")))
    s.focusMagnify = *v;
...
bool WifiServer::start(int port, const std::string &captureDir,
  ...
  std::cout << "WifiServer: listening on port " << port_ << ", serving "
            << captureDir_ << "\n";
  return true;
}
...
void WifiServer::handleConnection(int clientFd) {
  ...
  if (req.method == "GET" && req.path == "/") {
    ...
    int bat =
        batteryPercent_ ? batteryPercent_->load(std::memory_order_acquire) : 0;
    uint32_t cap =
