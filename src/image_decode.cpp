...
int decodePngCore(png_structp png, png_infop *outInfo, FILE *fp,
                  uint8_t **outRgb, png_bytep **outRows,
    ...
    *outInfo = info;
    ...
    if (width == 0 || height == 0 || width > kMaxDecodeDim || height > kMaxDecodeDim)
        return -1;
    ...
    *outRgb = rgb;  // Track for cleanup on longjmp
    ...
    png_bytep *rows = static_cast<png_bytep *>(std::malloc(static_cast<size_t>(height) * sizeof(png_bytep)));
    if (!rows) return -1;
...
int decodeJpegCore(struct jpeg_decompress_struct *cinfo, JpegErrCtx *jerr, FILE *fp,
                   uint8_t **outRgb, uint32_t *w, uint32_t *h, bool *outCreated) {
    *outRgb = nullptr;
    *outCreated = false;
    ...
    *outCreated = true;
    ...
    if (jpeg_read_header(cinfo, TRUE) != JPEG_HEADER_OK) return -1;

    cinfo->out_color_space = JCS_RGB;
    if (!jpeg_start_decompress(cinfo)) return -1;
    ...
    *outRgb = rgb;  // Track for cleanup on longjmp
    ...
    while (cinfo->output_scanline < height) {
        JSAMPROW rowPtr = rgb + static_cast<size_t>(cinfo->output_scanline) * rowBytes;
        JDIMENSION n = jpeg_read_scanlines(cinfo, &rowPtr, 1);
        if (n == 0) break;
        scanlinesRead += n;
        if (scanlinesRead > height) break;
    }
    ...
    if (!jpeg_finish_decompress(cinfo)) return -1;  // rgb freed by caller
...
std::vector<uint8_t> rgb24ToRgb565Scaled(const uint8_t *rgb,
                                          uint32_t srcW, uint32_t srcH,
                                          uint32_t dispW, uint32_t dispH) {
    if (srcW == 0 || srcH == 0 || dispW == 0 || dispH == 0) return {};
    size_t outSize = 0;
    if (!checkedMul(static_cast<size_t>(dispW), dispH, outSize)) return {};
    if (!checkedMul(outSize, 2, outSize)) return {};
    ...
    for (uint32_t dy = 0; dy < dispH; ++dy) {
        uint32_t sy = cropY + static_cast<uint32_t>((static_cast<uint64_t>(dy) * cropH) / dispH);
        if (sy >= srcH) sy = srcH - 1;
        for (uint32_t dx = 0; dx < dispW; ++dx) {
            uint32_t sx = cropX + static_cast<uint32_t>((static_cast<uint64_t>(dx) * cropW) / dispW);
...
uint8_t *decodePngToRgb24(FILE *fp, uint32_t &w, uint32_t &h) {
    ...
    std::free(static_cast<void *>(rows));
    if (rc != 0) { std::free(rgb); return nullptr; }
...
uint8_t *decodeJpegToRgb24(FILE *fp, uint32_t &w, uint32_t &h) {
    ...
    JpegErrCtx jerr;  // Must outlive cinfo (jpeg_destroy_decompress accesses cinfo->err)
    uint8_t *rgb = nullptr;
    ...
    if (cinfoCreated) jpeg_destroy_decompress(&cinfo);
...
std::vector<uint8_t> decodeImageToRgb565(const std::string &path,
    ...
    if (!rgb24 || w == 0 || h == 0) { std::free(rgb24); return {}; }
