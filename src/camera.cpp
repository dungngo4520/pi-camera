...
bool CameraApp::capture(const std::string &filename,
    ...
    public:
        ...
        ~SignalDisconnectGuard() noexcept {
            if (cam_) {
                try { cam_->requestCompleted.disconnect(); }
    ...
    handle_.camera()->requestCompleted.connect(this, [this, filename, warmup](Request *r) {
    ...
    long long exposureSecs = config_.exposureTime > 0
        ? std::min<long long>(static_cast<long long>(config_.exposureTime / 1000000LL), 3600LL)
        : 0;
    auto timeout = std::chrono::seconds(
        exposureSecs + 90 + static_cast<long long>(warmup) * 3);
    ...
    bool saved = false;
    {
        ...
        if (timedOut) {
            std::cerr << "Capture timed out (completed " << capCompleted_.load(std::memory_order_relaxed)
                      << "/" << (warmup + 1) << " frames)\n";
        ...
        if (saved && actualPath) *actualPath = capActualPath_;  // also under mutex
    }
...
void CameraApp::onCaptureCompleted(Request *r, const std::string &filename,
    ...
    uint32_t completed = capCompleted_.fetch_add(1, std::memory_order_relaxed) + 1;
    if (completed <= warmup) {
...
void CameraApp::stopCamera() noexcept {
    ...
    try {
        ...
        if (!callbacksCv_.wait_for(lk, std::chrono::seconds(60),
            ...
            std::cerr << "CameraApp: FATAL: callbacks stuck after 60s — requesting shutdown\n";
    ...
    if (manualExposure) {
        ...
        if (cfg.exposureTime > 0) {
            ...
            ctrls.set(controls::ExposureTime,
                      static_cast<int32_t>(std::min<uint64_t>(cfg.exposureTime,
                                                               INT32_MAX)));
        }
