...
bool runTimelapse(CameraApp &app, int intervalSec, int count,
                  const std::string &pattern, const std::string &captureDir) {
  ...
  for (int i = 0; infinite || i < count; ++i) {
    ...
    if (useDir) {
      ...
      if (!isFilePathInsideDir(fullPath, captureDir)) {
        std::cerr << "Timelapse path escapes capture directory: " << fullPath
                  << "\n";
        return false;
      }
    }
    ...
    std::cout << "[" << (i + 1) << "/" << shotLabel << "] " << fullPath << "\n";
    ...
    if ((infinite || i < count - 1) && !stop.stopRequested()) {
      ...
      auto end =
          std::chrono::steady_clock::now() + std::chrono::seconds(intervalSec);
