...
bool parseBtCommand(std::string_view line, BtCommand &out) {
    ...
    static const struct { const char *name; BtCommandType type; } kVerbs[] = {
        {"CAPTURE", BtCommandType::Capture},
        {"STATUS", BtCommandType::Status},
        {"SETTINGS", BtCommandType::Settings},
        {"LIST", BtCommandType::List},
        {"QUIT", BtCommandType::Quit},
    };
    for (const auto &v : kVerbs) {
        if (ieq(verb, v.name)) { out.type = v.type; return true; }
    }
            ...
            switch (cmd.type) {
                ...
                case BtCommandType::Status: {
                    ...
                    int bat = batteryPercent_
                        ? batteryPercent_->load(std::memory_order_acquire) : 0;
                    uint32_t cap = captureCount_
                        ? captureCount_->load(std::memory_order_acquire) : 0;
                    ...
                    try {
                        ...
                        if (fs::exists(captureDir_)) {
                            for (const auto &entry :
                                 fs::directory_iterator(captureDir_)) {
                        ...
                        std::cerr << "BtServer: directory listing failed: "
                                  << e.what() << "\n";
                    }
