...
std::string safeCapturePath(std::string_view rootDir,
    ...
    if (!isSafePathComponent(prefix)) return {};
    if (timestamp.empty()) return {};
    // Timestamp should be safe (generated internally), but validate anyway
    for (char c : timestamp) {
        if (c == '/' || c == '.' || static_cast<unsigned char>(c) < 0x20) return {};
    }
    // Extension should not contain path separators
    for (char c : ext) {
        if (c == '/' || static_cast<unsigned char>(c) < 0x20) return {};
    }

    std::string path;
    path.reserve(rootDir.size() + 1 + prefix.size() + 1 + timestamp.size() + 1 + ext.size());
    path.append(rootDir);
    ...
    if (path.size() >= 4096) return {};  // PATH_MAX includes NUL terminator
    return path;
}
...
bool isPathInside(std::string_view path, std::string_view root) {
    ...
    if (root.back() != '/' && path[root.size()] != '/') return false;
...
bool isSafeDevicePath(std::string_view path) {
    if (path.substr(0, 5) != "/dev/") return false;
    ...
    if (path.substr(0, 11) == "/dev/spidev") {
        ...
        if (bus.empty() || dev.empty()) return false;
        auto isDigit = [](char c) { return c >= '0' && c <= '9'; };
        if (!std::all_of(bus.begin(), bus.end(), isDigit)) return false;
        if (!std::all_of(dev.begin(), dev.end(), isDigit)) return false;
        return true;
    }
    // /dev/i2c-<digits>
...
int safeFileOpenFd(const std::string &path, std::string &outPath) {
    ...
    for (int i = 1; i <= 999; ++i) {
        ...
        if (errno != EEXIST && errno != ELOOP) return -1;
    }
    return -1;
}
...
std::string canonicalizeDir(const std::string &dir) {
    ...
    try {
        ...
        while (start <= result.size()) {
            ...
            std::string_view comp = (end == std::string::npos)
...
bool isCanonicalPathInside(const std::string &path, const std::string &root) {
    ...
    if (path == root) return false;  // must be strictly inside, not the root itself
...
bool isFilePathInsideDir(const std::string &filePath, const std::string &rootDir) {
    if (filePath.empty() || rootDir.empty()) return false;
    if (rootDir[0] != '/') return false;  // rootDir must be canonical (absolute)
    try {
        namespace fs = std::filesystem;
        fs::path fp(filePath);
        ...
        if (canonParentStr == rootDir) return true;
