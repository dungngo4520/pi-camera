...
// Pure-logic form of the path-traversal guard in
// WifiServer::handleConnection(); rejects '/' and ".." to prevent serving files
// outside captureDir.
...
public:
  ...
  bool start(int port, const std::string &captureDir, CameraSettings &settings,
             std::mutex &settingsMtx, std::atomic<bool> &captureRequest,
