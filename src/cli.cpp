...
uint32_t parseUint32(const char *s) {
  ...
  if (std::isspace(static_cast<unsigned char>(s[0])))
    throw std::invalid_argument(
        std::string("leading whitespace not allowed: '") + s + "'");
  if (s[0] == '-' || s[0] == '+')
  ...
  if (s[pos] != '\0') {
    throw std::invalid_argument(
        std::string("trailing characters after value: '") + s + "'");
  }
...
void printUsage(const char *prog) {
  std::cerr
      << "Pi Zero 2 WH + HQ Camera — libcamera C++ capture\n\n"
      << "Usage:\n"
      << "  " << prog
      << " --capture [file]              Capture a still (default: "
         "capture.<ext>)\n"
      << "  " << prog << " --list-controls               List camera controls\n"
      << "  " << prog << " --timelapse <sec> [options]   Timelapse mode\n"
      << "  " << prog
      << " --preview [options]           Live preview to SPI LCD (Waveshare "
         "1.44\" HAT)\n\n"
      << "Options:\n"
      << "  --format <type>         Output format for --capture/--timelapse: "
         "ppm, raw, png, jpeg, dng,\n"
      << "                          dng+jpeg, raw+jpeg (default: ppm). For "
         "--preview stills, use --capture-format.\n"
      << "                          jpeg = ISP hardware-encoded (Pi only), "
         "~10x faster\n"
      << "                          dng  = raw Bayer DNG (10-bit, for raw "
         "development)\n"
      << "                          dng+jpeg = DNG + JPEG pair (sequential "
         "capture)\n"
      << "                          raw+jpeg = NV12 raw + JPEG pair\n"
      << "  --png-level <0-9>       PNG compression level (0=none, 1=fast, "
         "6=default, 9=best)\n"
      << "  --jpeg-quality <1-100>  JPEG quality for software encode (default: "
         "90)\n"
      << "  --bracket <n,ev...>     HDR bracketing: capture N frames at EV "
         "offsets\n"
      << "                          e.g. --bracket 3,-2,0,+2 (3 frames: -2EV, "
         "0EV, +2EV)\n"
      << "  --output <pattern>      Filename pattern: %04d (sequence) or "
         "strftime (e.g. %F_%H%M%S)\n"
      ...
      << "  --warmup <n>            Frames to let AE/AWB converge (default: "
         "8)\n"
      ...
      << "  --capture-format <type> Still capture format: jpeg, png, ppm, dng, "
         "dng+jpeg, raw+jpeg (default: jpeg)\n"
      << "  --capture-dir <path>    Directory for captured images (default: "
         ".)\n"
      << "  --capture-prefix <str>  Filename prefix for captures (default: "
         "capture)\n"
      << "  --spi-device <path>     SPI device for display (default: "
         "/dev/spidev0.0)\n"
      << "  --display-rotate <deg>  Display rotation: 0, 90, 180, 270 "
         "(default: 90)\n"
      << "  --battery               Show battery level overlay on preview "
         "(ADS1115 ADC)\n"
      << "  --battery-i2c <path>    I2C device for ADS1115 (default: "
         "/dev/i2c-1)\n"
      << "  --battery-addr <hex>    ADS1115 I2C address (default: 0x48)\n"
      << "  --wifi                  Enable Wi-Fi HTTP server (port 8080) for "
         "remote control\n"
      << "                          and image transfer (GET / lists files, "
         "POST /capture triggers shot)\n"
      << "  --bt                    Enable Bluetooth RFCOMM serial remote "
         "control (channel 1)\n"
      << "                          Text commands: CAPTURE, STATUS, SETTINGS, "
         "SET key=val, LIST, QUIT\n\n"
      << "Examples:\n"
      << "  " << prog << " --capture photo.png --format png\n"
      << "  " << prog
      << " --capture photo.jpg --format jpeg       (ISP hardware encode, ~10x "
         "faster)\n"
      << "  " << prog << " --capture photo.raw --format raw\n"
      << "  " << prog << " --capture photo.ppm --iso 2.0 --shutter 30000\n"
      << "  " << prog
      << " --timelapse 60 --count 10 --output timelapse_%04d.png --format png\n"
      << "  " << prog
      << " --preview                              (live preview, "
         "joystick=capture)\n"
      << "  " << prog
      << " --preview --display-rotate 90          (rotated display)\n"
      << "  " << prog
      << " --preview --capture-format png         (capture PNG on button "
         "press)\n"
      << "  " << prog
      << " --preview --battery                   (show battery % on LCD)\n";
}
...
bool parseArgs(int argc, char **argv, CliOptions &opts, CameraConfig &cfg) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--capture") {
      ...
      opts.mode = "timelapse";
      if (!parseIntArg(
              argc, argv, i, "--timelapse",
              [](const char *s) { return parseInt32(s); },
              opts.timelapseInterval))
        return false;
    } else if (arg == "--output") {
      if (!parseStrArg(argc, argv, i, "--output", opts.outputPattern))
        return false;
    } else if (arg == "--count") {
      if (!parseIntArg(
              argc, argv, i, "--count",
              [](const char *s) { return parseInt32(s); }, opts.timelapseCount))
        return false;
    } else if (arg == "--width") {
      if (!parseIntArg(argc, argv, i, "--width", parseUint32, cfg.width))
        return false;
    } else if (arg == "--height") {
      if (!parseIntArg(argc, argv, i, "--height", parseUint32, cfg.height))
        return false;
    } else if (arg == "--iso") {
      if (!parseIntArg(
              argc, argv, i, "--iso",
              [](const char *s) { return parseFloat(s); }, cfg.analogueGain))
        return false;
      ...
      if (!parseIntArg(
              argc, argv, i, "--digital-gain",
              [](const char *s) { return parseFloat(s); }, cfg.digitalGain))
        return false;
      ...
      std::string fmt;
      if (!parseStrArg(argc, argv, i, "--format", fmt))
        return false;
      auto parsed = parseOutputFormat(fmt);
      if (!parsed) {
        std::cerr
            << "Unknown format: " << fmt
            << " (options: ppm, raw, png, jpeg, dng, dng+jpeg, raw+jpeg)\n";
        return false;
      }
      cfg.format = *parsed;
    } else if (arg == "--png-level") {
      if (!parseIntArg(
              argc, argv, i, "--png-level",
              [](const char *s) { return parseInt32(s); }, cfg.pngLevel))
        return false;
      ...
      if (!parseIntArg(
              argc, argv, i, "--jpeg-quality",
              [](const char *s) { return parseInt32(s); }, cfg.jpegQuality))
        return false;
      ...
      if (!parseStrArg(argc, argv, i, "--spi-device", opts.spiDevice))
        return false;
    } else if (arg == "--display-rotate") {
      if (!parseIntArg(
              argc, argv, i, "--display-rotate",
              [](const char *s) { return parseInt32(s); },
      ...
      std::string fmt;
      if (!parseStrArg(argc, argv, i, "--capture-format", fmt))
        return false;
      if (!parseOutputFormat(fmt)) {
        std::cerr
            << "Unknown --capture-format: " << fmt
            << " (options: jpeg, png, ppm, raw, dng, dng+jpeg, raw+jpeg)\n";
        return false;
      }
      opts.captureFormat = fmt;
    } else if (arg == "--capture-dir") {
      if (!parseStrArg(argc, argv, i, "--capture-dir", opts.captureDir))
        return false;
    } else if (arg == "--capture-prefix") {
      if (!parseStrArg(argc, argv, i, "--capture-prefix", opts.capturePrefix))
        return false;
    } else if (arg == "--warmup") {
      ...
      if (!parseStrArg(argc, argv, i, "--battery-i2c", opts.batteryI2cDevice))
        return false;
    } else if (arg == "--battery-addr") {
      if (!parseIntArg(argc, argv, i, "--battery-addr", parseUint8Hex,
                       opts.batteryI2cAddress))
        return false;
    } else if (arg == "--wifi") {
  ...
  if (cfg.width == 0 || cfg.width > kMaxSensorWidth) {
    std::cerr << "--width must be 1-" << kMaxSensorWidth << ", got "
              << cfg.width << "\n";
    return false;
  }
  if (cfg.height == 0 || cfg.height > kMaxSensorHeight) {
    std::cerr << "--height must be 1-" << kMaxSensorHeight << ", got "
              << cfg.height << "\n";
    return false;
  }
  if (opts.previewWidth == 0 || opts.previewWidth > kMaxSensorWidth) {
    std::cerr << "--preview-w must be 1-" << kMaxSensorWidth << ", got "
              << opts.previewWidth << "\n";
    return false;
  }
  if (opts.previewHeight == 0 || opts.previewHeight > kMaxSensorHeight) {
    std::cerr << "--preview-h must be 1-" << kMaxSensorHeight << ", got "
              << opts.previewHeight << "\n";
    return false;
  }
  if (opts.captureWidth == 0 || opts.captureWidth > kMaxSensorWidth) {
    std::cerr << "--capture-w must be 1-" << kMaxSensorWidth << ", got "
              << opts.captureWidth << "\n";
    return false;
  }
  if (opts.captureHeight == 0 || opts.captureHeight > kMaxSensorHeight) {
    std::cerr << "--capture-h must be 1-" << kMaxSensorHeight << ", got "
              << opts.captureHeight << "\n";
    return false;
  }
  ...
  if (opts.batteryI2cAddress < 0x03 || opts.batteryI2cAddress > 0x77) {
    std::cerr << "--battery-addr must be 0x03-0x77, got 0x" << std::hex
              << static_cast<int>(opts.batteryI2cAddress) << std::dec << "\n";
    return false;
  }
  ...
  if (!isSafePathComponent(opts.capturePrefix)) {
    std::cerr << "--capture-prefix contains unsafe characters (.., /, control "
                 "chars)\n";
    return false;
  }
  ...
  if (!opts.captureFile.empty() && !isSafeFilePath(opts.captureFile)) {
    std::cerr << "--capture contains unsafe path components (.., absolute, "
                 "control chars)\n";
    return false;
  }
  ...
  if (!opts.outputPattern.empty() && !isSafeFilePath(opts.outputPattern)) {
    std::cerr << "--output contains unsafe path components (.., absolute, "
                 "control chars)\n";
    return false;
  }
