...
// libcamera::controls::AwbModeEnum (see libcamera/control_ids.h). kelvinToGains()
// converts a colour temperature to approximate R/B gains. Both are pure logic
// with no libcamera dependency, so they live in this header — which is part of
// the picamera_core static library — and are unit-testable on x86 builds that
// do not have libcamera installed.
//
...
// Lower K (warm) -> more red, less blue. Higher K (cool) -> less red, more blue.
// Gains are clamped to [0.1, 8.0]. Pure logic (unit-testable on x86).
