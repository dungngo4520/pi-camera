...
public:
  ...
  bool init();
  bool start(uint32_t vfW, uint32_t vfH, uint32_t capW, uint32_t capH,
             OutputFormat capFmt);
  void stop() noexcept;
  ...
  bool waitCaptureDone(int timeoutMs = 5000, std::string *savedPath = nullptr);
  ...
  bool reconfigureStill(uint32_t vfW, uint32_t vfH, uint32_t capW,
  ...
  uint32_t lastShutterMs() const {
    return lastShutterMs_.load(std::memory_order_acquire);
  }
