...
TEST(draw_char_scaled_2x_produces_larger_glyph) {
    ...
    int count1x = 0, count2x = 0;
    for (size_t i = 0; i < sizeof(buf1x); i += 2) {
        if (buf1x[i] != 0 || buf1x[i + 1] != 0) ++count1x;
        if (buf2x[i] != 0 || buf2x[i + 1] != 0) ++count2x;
    }
...
TEST(draw_char_transparent_leaves_background_untouched) {
    ...
    bool hasChanged = false, hasUnchanged = false;
...
TEST(draw_char_negative_coords_no_crash) {
    ...
    bool hasPixel = false;
    for (size_t i = 0; i < sizeof(buf); i += 2) {
        if (buf[i] != 0 || buf[i + 1] != 0) { hasPixel = true; break; }
    }
...
TEST(draw_battery_icon_high_percent_uses_green) {
    ...
    for (int y = 1; y <= 7; ++y) {
        for (int x = 1; x <= 14; ++x) {
            ...
            if (px == kColorGreen) { hasGreen = true; break; }
        }
    }
    CHECK(hasGreen);
}
...
TEST(draw_battery_icon_mid_percent_uses_yellow) {
    ...
    for (int y = 1; y <= 7; ++y) {
        for (int x = 1; x <= 14; ++x) {
            size_t idx = (y * W + x) * 2;
            uint16_t px = (buf[idx] << 8) | buf[idx + 1];
            if (px == kColorYellow) { hasYellow = true; break; }
        }
    }
    CHECK(hasYellow);
}
...
TEST(draw_battery_icon_low_percent_uses_red) {
    ...
    for (int y = 1; y <= 7; ++y) {
        for (int x = 1; x <= 14; ++x) {
            size_t idx = (y * W + x) * 2;
            uint16_t px = (buf[idx] << 8) | buf[idx + 1];
            if (px == kColorRed) { hasRed = true; break; }
        }
    }
    CHECK(hasRed);
}
