...
static size_t findIfdEntry(const std::vector<uint8_t> &buf, size_t ifdOff,
                           uint16_t tag) {
...
TEST(exif_contains_make_entry) {
  ...
  CHECK(count == 13);
  // The offset points into the TIFF data area (relative to TIFF start = buffer
  // offset 6)
...
TEST(exif_exposure_time_rational) {
  ...
  // Wait: GCD(8000, 1000000) = 1000? Let me check: 1000000 / 8000 = 125, so
  // 8000*125=1000000. GCD = 8000. So 8000/8000=1, 1000000/8000=125. Result:
  // 1/125.
  CHECK(num == 1);
  CHECK(den == 125);
}
