...
FrameView makeNv12Frame(const std::vector<uint8_t> &y,
                        const std::vector<uint8_t> &uv, uint32_t w,
                        uint32_t h) {
  FrameView f;
  f.width = w;
  f.height = h;
...
TEST(hwjpeg_writer_truncates_at_ffd9) {
  ...
  std::vector<uint8_t> fakeJpeg = {
      0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 'J', 'F', 'I',
      'F',  0xFF, 0xD9, 0x00, 0x00, 0xDE, 0xAD // padding that must be stripped
  };
  FrameView f;
  f.width = 2;
  f.height = 2;
...
TEST(dng_writer_produces_valid_tiff_header) {
  ...
  std::vector<uint8_t> rawBayer = {0x10, 0x20, 0x30, 0x40, 0x00,
