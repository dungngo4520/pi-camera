...
TEST(draw_aspect_ratio_mask_native_no_op) {
    ...
    for (size_t i = 0; i < fb.size(); ++i) fb.data[i] = 0x55;
...
TEST(draw_grid_golden_ratio_draws_lines) {
    ...
    for (uint32_t y = 0; y < fb.h; ++y) {
        if (fb.pixel(g1, y) != 0) foundG1 = true;
        if (fb.pixel(g2, y) != 0) foundG2 = true;
    }
    CHECK(foundG1);
    CHECK(foundG2);
}
...
TEST(draw_zebra_marks_bright_pixels) {
    ...
    const uint32_t w = 16, h = 16, stride = 16;
    std::vector<uint8_t> y(stride * h, 255);
    drawZebra(fb.ptr(), fb.w, fb.h, fb.size(), y.data(), w, h, stride, y.size(), 200);
...
TEST(draw_focus_peaking_marks_edges) {
    ...
    for (uint32_t row = 0; row < h; ++row)
        for (uint32_t x = 8; x < w; ++x)
            y[row * stride + x] = 255;
    drawFocusPeaking(fb.ptr(), fb.w, fb.h, fb.size(), y.data(), w, h, stride, y.size());
...
TEST(draw_bulb_timer_no_crash_on_zero_dims) {
    drawBulbTimer(nullptr, 0, 0, 5);
}
...
void setSrcPixel(std::vector<uint8_t> &img, uint32_t imgW,
...
TEST(draw_image_view_zoomed_1x_copies_image) {
    ...
    drawImageViewZoomed(fb.ptr(), fb.w, fb.h, fb.size(),
                        img.data(), img.size(), fb.w, fb.h,
                        1, 0, 0, "/x/y.jpg");
...
TEST(draw_image_view_zoomed_zero_dimensions_no_crash) {
    ...
    drawImageViewZoomed(buf.data(), 0, 0, buf.size(),
                        img.data(), img.size(), 128, 128,
                        2, 0, 0, "/x.jpg");
    // No crash = pass.
}
