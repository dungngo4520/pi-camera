...
TEST(dng_writes_valid_tiff_header) {
    ...
    bool foundWidth = false, foundHeight = false, foundCompression = false;
    for (uint16_t i = 0; i < tagCount; ++i) {
        ...
        if (tag == 256) { CHECK_EQ(val, 4u); foundWidth = true; }
        if (tag == 257) { CHECK_EQ(val, 4u); foundHeight = true; }
        if (tag == 259) { CHECK_EQ(val, 1u); foundCompression = true; } // uncompressed
    }
    CHECK(foundWidth);
    CHECK(foundHeight);
...
TEST(dng_raw_data_preserved) {
    ...
    for (uint16_t i = 0; i < tagCount; ++i) {
        ...
        uint16_t tag = readU16(&buf[entryOff]);
        if (tag == 273) stripOffset = readU32(&buf[entryOff + 8]);
        if (tag == 279) stripByteCount = readU32(&buf[entryOff + 8]);
    }
...
TEST(dng_exif_metadata_embedded) {
    ...
    bool foundExposure = false, foundIso = false;
