...
TEST(settings_tab_shooting_has_16_items) {
    CHECK(settingsTabItemCount(SettingsTab::Shooting) == 16);
}

TEST(settings_tab_image_has_17_items) {
    CHECK(settingsTabItemCount(SettingsTab::Image) == 17);
}

TEST(settings_tab_display_has_6_items) {
    CHECK(settingsTabItemCount(SettingsTab::Display) == 6);
}
...
TEST(settings_value_ev_default_zero) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 5, s) == "0");
}
...
TEST(settings_adjust_ev_increases) {
    ...
    settingsItemAdjustRight(SettingsTab::Shooting, 5, s);
    CHECK(s.exposureValue > 0);
}
...
TEST(settings_adjust_ev_decreases) {
    ...
    settingsItemAdjustLeft(SettingsTab::Shooting, 5, s);
...
TEST(settings_adjust_awb_cycles_through_modes) {
    ...
    settingsItemAdjustRight(SettingsTab::Image, 4, s);
...
TEST(settings_adjust_wb_red_clamps_min) {
    ...
    settingsItemAdjustLeft(SettingsTab::Image, 6, s);
...
TEST(settings_adjust_sharpness_clamps_max) {
    CameraSettings s;
    s.sharpness = 15.8f;
    settingsItemAdjustRight(SettingsTab::Image, 11, s);
...
TEST(settings_adjust_brightness_clamps_min) {
    CameraSettings s;
    s.brightness = -0.95f;
    settingsItemAdjustLeft(SettingsTab::Image, 8, s);
...
TEST(settings_adjust_awb_unknown_mode_falls_back_to_auto) {
    ...
    settingsItemAdjustLeft(SettingsTab::Image, 4, s);
...
TEST(settings_adjust_exposure_mode_cycles) {
    ...
    settingsItemAdjustRight(SettingsTab::Shooting, 14, s);
...
TEST(settings_exposure_mode_program_reenables_ae) {
    ...
    s.shutterUs = 1000;
    s.analogueGain = 2.0f;
    settingsItemAdjustRight(SettingsTab::Shooting, 14, s); // Auto
    settingsItemAdjustRight(SettingsTab::Shooting, 14, s); // Program
    CHECK(s.exposureMode == ExposureMode::Program);
    CHECK(s.aeEnable == true);
    CHECK(s.shutterUs == 0);
    CHECK(s.analogueGain == 0);
...
TEST(settings_value_exposure_mode_default) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 14, s) == "P");
}
...
TEST(settings_value_bracket_type_default_ae) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 15, s) == "AE");
}

TEST(settings_value_bracket_type_iso) {
    CameraSettings s;
    s.bracketType = BracketType::ISO;
    CHECK(settingsItemValue(SettingsTab::Shooting, 15, s) == "ISO");
}
...
TEST(settings_adjust_picture_style_cycles) {
    ...
    settingsItemAdjustRight(SettingsTab::Image, 15, s);
    CHECK(s.pictureStyle == PictureStyle::Neutral);
    settingsItemAdjustRight(SettingsTab::Image, 15, s);
...
TEST(settings_value_picture_style_default) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Image, 15, s) == "STD");
}
...
TEST(settings_label_image_wbset) {
    CHECK(settingsItemLabel(SettingsTab::Image, 16) == "WBSET");
}

TEST(settings_value_wbset_default_off) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Image, 16, s) == "OFF");
}
...
TEST(settings_value_wbset_shows_set_when_pending) {
    ...
    CHECK(settingsItemValue(SettingsTab::Image, 16, s) == "SET");
}
...
TEST(settings_value_aemode_default_normal) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 7, s) == "NORMAL");
}

TEST(settings_value_aemode_short) {
    CameraSettings s;
    s.aeExposureMode = AeExposureMode::Short;
    CHECK(settingsItemValue(SettingsTab::Shooting, 7, s) == "SHORT");
}

TEST(settings_value_aemode_long) {
    CameraSettings s;
    s.aeExposureMode = AeExposureMode::Long;
    CHECK(settingsItemValue(SettingsTab::Shooting, 7, s) == "LONG");
}
...
TEST(settings_value_aeconst_highlight) {
    ...
    CHECK(settingsItemValue(SettingsTab::Shooting, 8, s) == "HIGHLIGHT");
}

TEST(settings_value_aeconst_shadows) {
    CameraSettings s;
    s.aeConstraintMode = AeConstraintMode::Shadows;
    CHECK(settingsItemValue(SettingsTab::Shooting, 8, s) == "SHADOWS");
}
...
TEST(settings_value_interval_default) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 12, s) == "5S");
}

TEST(settings_value_interval_custom) {
    CameraSettings s;
    s.timelapseInterval = 60;
    CHECK(settingsItemValue(SettingsTab::Shooting, 12, s) == "60S");
}
...
TEST(settings_adjust_count_increases) {
    ...
    settingsItemAdjustRight(SettingsTab::Shooting, 13, s);
...
TEST(settings_adjust_count_decreases) {
    CameraSettings s;
    CHECK(s.timelapseCount == 10);
    settingsItemAdjustLeft(SettingsTab::Shooting, 13, s);
...
TEST(settings_value_count_default) {
    CameraSettings s;
    CHECK(settingsItemValue(SettingsTab::Shooting, 13, s) == "10");
}

TEST(settings_value_count_zero_shows_inf) {
    CameraSettings s;
    s.timelapseCount = 0;
    CHECK(settingsItemValue(SettingsTab::Shooting, 13, s) == "INF");
}
...
TEST(settings_adjust_nr_cycles_right) {
    ...
    settingsItemAdjustRight(SettingsTab::Image, 12, s);
...
TEST(settings_adjust_nr_cycles_left) {
    ...
    settingsItemAdjustLeft(SettingsTab::Image, 12, s);
    CHECK(s.noiseReduction == NoiseReductionMode::Off);
    // Off -> Minimal
    settingsItemAdjustLeft(SettingsTab::Image, 12, s);
    CHECK(s.noiseReduction == NoiseReductionMode::Minimal);
